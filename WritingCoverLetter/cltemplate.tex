%!TEX program = xelatex 

\documentclass[$size$]{letter}
\usepackage[letterpaper,margin=1in]{geometry}

\usepackage{fixltx2e}

% Adds microtypography features: character protrusion, for XeTeX.
\usepackage{microtype}

% Allows links.
\usepackage[usenames,dvipsnames]{color}
\usepackage[xetex, bookmarks, colorlinks, breaklinks]{hyperref}
\hypersetup{colorlinks=true, urlcolor=NavyBlue, linkcolor=NavyBlue}

\urlstyle{rm}

\frenchspacing{}        % Remove extra spaces after periods.
\pagestyle{empty}       % Remove page numbers.

\usepackage{fontspec}
\defaultfontfeatures{Mapping=tex-text}  % Sets quotation marks.
\setromanfont{Fira Sans Condensed Medium}
\setsansfont[BoldFont={Fira Sans Condensed Bold}]{Fira Sans Condensed Medium}
\setmonofont[Scale=MatchLowercase]{Fira Mono Medium}

% From default template
\providecommand{\tightlist}{%
  \setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}}

\newcommand\shyp{\-}

\newcommand{\header}[9]{
    {\hypersetup{hidelinks}
    \noindent
    \parbox[b][0.625in][t]{6.5in}{
        \vspace{-0.375in}
        \parbox[b][0.625in][t]{3.74in}
            {\href{http://www.umich.edu}
            {\includegraphics[height=0.5in]{./UO.pdf}}}\hfill
        \parbox[b][0.625in][t]{2in}{
            \setlength{\baselineskip}{9pt}
            \fontsize{7pt}{7pt}{
                \fontspec{Fira Sans Condensed Bold}
                \textbf{#8}\\
                {#9}\vspace{6pt}\\
                #1\\
                #2, #3 #4\\
                Phone: #5\\
                Email: \href{mailto:#7}{#7}
                }
            }
        }}
    }

$if(draft)$
\usepackage{draftwatermark}
\SetWatermarkLightness{1}
$endif$

\begin{document}

\header{$head.address$}{$head.city$}{$head.state$}{$head.zip$}{$head.phone$}{$head.fax$}{$head.email$}{$sig.name$}{$head.dept$}


\vspace{0.5in}

$body$

$if(sig.include)$
    \begin{minipage}[t]{6.5in}
    Sincerely,\\
    
    \vspace{0.1in}

    $if(sig.sign)$
        \vspace{-0.2in}\includegraphics[height=0.001in]{./Signature.pdf}
    $else$
        \\
    $endif$
    \\
    $sig.name$\\
    $sig.post$
    $if(sig.secondpost)$
        \\
        $sig.secondpost$
    $endif$
    \end{minipage}
$endif$

$if(cc)$
    cc: $for(cc)$$cc$$sep$, $endfor$
$endif$

$if(ps)$
    $ps$
$endif$

\end{document}